...
export function resolveTurn(state, actions) {
    ...
    // ============================================================================
    const interaction = resolveInteractionMatrix(state, state.players[0].id, player1Action, state.players[1].id, player2Action);
    // ============================================================================
    ...
    if (interaction.player1Card) {
        newPlayers[0] = addCardToPlayerLane(newPlayers[0], interaction.player1Card, interaction.player1TargetLane);
    }
    // Apply cards for player 2
    if (interaction.player2Card) {
        newPlayers[1] = addCardToPlayerLane(newPlayers[1], interaction.player2Card, interaction.player2TargetLane);
    }
    // Apply Blind Hit cards
    if (p1BlindCard && player1Action.type === 'blind_hit') {
        newPlayers[0] = addCardToPlayerLane(newPlayers[0], p1BlindCard, player1Action.targetLane);
    }
    if (p2BlindCard && player2Action.type === 'blind_hit') {
        newPlayers[1] = addCardToPlayerLane(newPlayers[1], p2BlindCard, player2Action.targetLane);
    }
    // Apply energy costs
    newPlayers[0] = {
        ...newPlayers[0],
    ...
    // ============================================================================
    const allLanesLocked = newPlayers.every(player => player.lanes.every(lane => lane.locked));
 ...
 */
function resolveInteractionMatrix(state, player1Id, player1Action, player2Id, player2Action) {
    ...
    if (p1Type === 'take' && p2Type === 'take') {
        ...
        result.player1TargetLane = player1Action.targetLane;
        result.player2Card = frontCard;
        result.player2TargetLane = player2Action.targetLane;
        return result;
    }
 ...
 */
function createAshCard(turnNumber, playerId) {
    return {
 ...
 */
function addCardToPlayerLane(player, card, laneIndex) {
 ...
 */
function lockPlayerLane(player, laneIndex) {
    const targetLane = player.lanes[laneIndex];
 ...
 */
function resolveLaneBustsAndLocks(player) {
 ...
 */
function compareLanes(lane1, lane2) {
 ...
 */
function resolveAuctionTurn(state, actions) {
    const p1Action = actions.playerActions.find(pa => pa.playerId === state.players[0].id)?.action;
    const p2Action = actions.playerActions.find(pa => pa.playerId === state.players[1].id)?.action;
...
function calculateTotalBoardScore(player) {
    return player.lanes.reduce((sum, lane) => sum + (lane.busted ? 0 : lane.total), 0);
}
function applyVoidStone(player, laneIndex, _turn) {
    const newLanes = [...player.lanes];
 ...
 */
function resolveOverheat(player, action) {
...
function checkAndApply21EnergyBonus(oldPlayer, newPlayer) {
