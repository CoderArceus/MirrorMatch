 ...
 */
export function chooseAction(state, playerId, difficulty) {
 ...
 */
function chooseEasyAction(legalActions) {
...
// ============================================================================
function scoreAction(state, player, opponent, action, config) {
    let score = 0;
    switch (action.type) {
        case 'take': {
            ...
            if (!oppLane.busted && newTotal > oppLane.total) {
                if (oppLane.locked)
                    score += config.LANE_WIN_LOCKED;
                else
                    score += config.LANE_WIN_LEADING;
            }
        ...
        case 'stand': {
            ...
            if (targetLane.total === 21)
                score += config.TOTAL_21;
            else if (targetLane.total >= 18)
                score += config.TOTAL_18_20;
            else if (targetLane.total >= 15)
                score += config.TOTAL_15_17;
            else
                score += config.LOCK_WEAK_LANE_PENALTY;
            // 2. Win Confirmation
            const oppLane = opponent.lanes[action.targetLane];
            if (!oppLane.busted && targetLane.total > oppLane.total) {
                if (oppLane.locked)
                    ...
                    score += config.LANE_WIN_PRESSURE;
            }
            // 3. Match Win pressure (2nd lane)
            const myLockedWins = player.lanes.filter((l, i) => l.locked && !l.busted && l.total > opponent.lanes[i].total).length;
        ...
        case 'burn': {
            ...
            // Energy penalty
            if (player.energy <= 1)
                score += config.ENERGY_CONSERVATION;
            break;
        }
        case 'pass': {
        ...
        case 'bid': {
            ...
            if (lane.locked) {
                ...
                // This is GOOD.
                if (lane.total < 17)
                    score += 100; // Second chance!
                else
                    score -= 100; // Don't unlock a winning lane!
            }
            else {
                // Open lane
                if (lane.total === 0)
                    score += 50; // Safe place
                else if (lane.total >= 18)
                    score -= 150; // Don't shackle high lane
            }
            break;
        }
        case 'blind_hit': {
            ...
            if (projectedTotal > 21)
                score += config.BUST_PENALTY; // Likely bust
            else if (projectedTotal >= 18 && projectedTotal <= 21)
                score += config.TOTAL_18_20;
            else
                score += -50; // Uncertainty penalty
            // Overheat cost
            score -= 50;
...
function chooseMediumAction(state, playerId, legalActions) {
    return chooseScoredAction(state, playerId, legalActions, MEDIUM_CONFIG);
}
function chooseHardAction(state, playerId, legalActions) {
...
function chooseScoredAction(state, playerId, legalActions, config) {
 ...
 */
function breakTie(scored, playerId) {
    if (scored.length === 0) {
